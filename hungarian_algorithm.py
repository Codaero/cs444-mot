...
def object_assign(curr_objs, prev_objs, frame_num):
    ...
    assignment = filter_detections(assignment)
    ...
    if len(curr_objs) > len(prev_objs):
        ...
        for row, col in zip(row_idx, col_idx):
            ...
            new_objs[col] = [new_objs[col][0], curr_objs[row][0], curr_objs[row][1], curr_objs[row][2], curr_objs[row][3], curr_objs[row][4], frame_num]

        next_new_id, curr_ids = new_objs[-1][0] + 1, len(curr_objs) - 1

        unidentified_ids = [x for x in curr_ids if x not in row]
